...
class KidneyConfig(Config):
    ...
    NUM_CLASSES = 1 + 1 + 1  # Background + bg + nucleus
...
class UltrasoundDataset(utils.Dataset):
    ...
    def load_kidney(self, dataset_dir, subset, seg_dir=None):
        ...
        if seg_dir:
            ...
            for id, name in enumerate(self.class_list, start=1):
                ...
                print("find mask id:{} name:{} count:{}".format(id, name, len(mask_class_dict)))
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        m = skimage.io.imread(mask_path, as_gray=True).astype(np.bool)
...
def detect(model, dataset_dir, subset):
    ...
    for image_id in dataset.image_ids:
        ...
        target_class_id = 2  # kidney

        target_ids = np.where(r['class_ids'] == target_class_id)
        target_ids = sorted(target_ids[0], key=lambda i: r['scores'][i], reverse=True)
        ...
        for idx in target_ids:
            ...
            cv2.imwrite(os.path.join(result_seg_dir, source_id + '#' + str(save_order) + '.png'), m)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seg_dir',
                        default='~/data/yonsei2/dataset/size512/SegKidney_v2_MRCNN',
